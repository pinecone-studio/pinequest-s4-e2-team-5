  ...
  return problems.map((p, i) => {
    ...
    if (shouldNormalizeSequence && (sequenceOps.length >= 2 || rawSequenceSlots.filter((v) => Number.isFinite(v)).length >= 2)) {
      ...
      if (slots.length && knownSlots.length >= 2 && slots.some((v: number | null) => v === null)) {
        ...
        const step = sequenceOps[sequenceOps.length - 1] - sequenceOps[sequenceOps.length - 2];
        const isArithmetic = sequenceOps.length < 3 || sequenceOps.every((n, idx) => idx === 0 || n - sequenceOps[idx - 1] === step);
        if (isArithmetic) {
          out.type = "number_sequence";
          out.answer = [1, 2, 3].map((k) => sequenceOps[sequenceOps.length - 1] + step * k);
          out.sequenceSlots = [...sequenceOps, null, null, null];
          out.missingPositions = [sequenceOps.length, sequenceOps.length + 1, sequenceOps.length + 2];
          out.missingPosition = sequenceOps.length;
          out.sequenceStep = step;
          out.answerCount = 3;
          out.operator = null;
          out.knownResult = null;
          out.operands = sequenceOps;
        } else {
          const prev = sequenceOps[sequenceOps.length - 2];
          const ratio = prev !== 0 ? sequenceOps[sequenceOps.length - 1] / prev : null;
          const isGeometric =
            ratio !== null &&
            Number.isFinite(ratio) &&
            sequenceOps.every((n, idx) => idx === 0 || (sequenceOps[idx - 1] !== 0 && n / sequenceOps[idx - 1] === ratio));
          if (isGeometric) {
            out.type = "number_sequence";
            out.answer = [1, 2, 3].map((k) => sequenceOps[sequenceOps.length - 1] * ratio ** k);
            out.sequenceSlots = [...sequenceOps, null, null, null];
            out.missingPositions = [sequenceOps.length, sequenceOps.length + 1, sequenceOps.length + 2];
            out.missingPosition = sequenceOps.length;
